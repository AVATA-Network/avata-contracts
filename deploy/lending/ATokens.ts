...
module.exports = async function (hre: HardhatRuntimeEnvironment) {
    ...
    for (const readyToken of _readyTokensForDeploy) {
        ...
        const { name, tokenAddress, interestRateModel, decimals, priceFeedAddress, collateralFactor, reserveFactor, initialExchangeRateMantissa } = aTokenConfig
        ...
        await execute('PriceOracleProxyUSD', { from: deployer, log: true }, '_setAggregators', 
            [aErc20Delegator.address],
            [priceFeedAddress]
        );

        await execute('Avatroller', { from: deployer, log: true }, '_setCollateralFactor', 
            aErc20Delegator.address,
            parseEther(collateralFactor)
        );
